DROP TABLE IF EXISTS medintsev.market_page_cpm_4search20perday;
CREATE TABLE medintsev.market_page_cpm_4search20perday
...
SELECT -- запрос рассчитывает среднее CPM_S каждой страницы (page_groupid_1 + page_groupid_2),
	...
	to_date(from_unixtime(visit_start_time)) as day,
	page_groupid_1,
	page_groupid_2,
...
FROM
	...
	SELECT -- запрос рассчитывает CPM_S для каждого визита
		visit_start_time, visit_id, hyper_id, page_groupid_1, page_groupid_2, AVG(pre_CPM_S) as CPM_S_visit
	FROM
		(
	...
	WHERE event = 'access'
	GROUP BY
		visit_start_time, visit_id, hyper_id, page_groupid_1, page_groupid_2
	) t
GROUP BY
	to_date(from_unixtime(visit_start_time)), page_groupid_1, page_groupid_2;